  ...
  // ✅ Full React Component: Secure, Fullscreen, Resume-enabled TestTaking
...
const TestTaking: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('ongoingTest');
    if (saved) {
      ...
      if (data.testId === id) {
        ...
        setTestStarted(true);
        toast('Resuming your previous test session');
      } else {
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      if (!document.fullscreenElement) {
        toast.error('You exited fullscreen. This may be flagged.');
      }
    };
  ...
  }, [answers, flagged, startTime, currentQuestion, timeLeft]);
  ...
  const startTest = async () => {
    ...
    try {
      ...
      setTest(prev => prev ? { ...prev, questions: response.data.questions } : null);
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to start test');
      setTestStarted(false);
    }
  ...
  const handleSubmitTest = async (autoSubmit = false) => {
    if (!testStarted || !test || !startTime) return;
    const confirmSubmit = autoSubmit || window.confirm('Are you sure you want to submit the test?');
    if (!confirmSubmit) return;
    ...
    try {
      ...
      await axios.post('/results/submit', {
        testId: test._id,
  ...
  if (!test) return <div className="text-center py-12"><AlertCircle className="h-12 w-12 text-red-500 mx-auto mb-4" /><h3 className="text-lg font-medium">Test not found</h3></div>;
        ...
        <div className="card p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <CheckCircle className="h-8 w-8 text-green-600 mx-auto mb-2" />
              <p className="text-sm text-gray-600">Questions</p>
              <p className="text-lg font-semibold">{test?.questions?.length || test?.questionCount}</p>
            </div>
            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <Flag className="h-8 w-8 text-purple-600 mx-auto mb-2" />
              <p className="text-sm text-gray-600">Total Marks</p>
          ...
          <div className="text-center">
            <button onClick={startTest} className="btn btn-primary px-8 py-3 text-lg">Start Test</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto" ref={testContainerRef}>{/* Test UI continues here... */}
     <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          {/* Question Panel */}
          <div className="lg:col-span-3">
            <div className="card p-6">
